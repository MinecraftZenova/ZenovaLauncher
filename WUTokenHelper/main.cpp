...
extern "C" __declspec(dllexport) int  __stdcall GetWUToken(wchar_t** retToken, wchar_t** accountId) {
	auto tokenBrokerStatics = get_activation_factory<TokenBrokerInternal, Windows::Foundation::IUnknown>();
	auto statics = tokenBrokerStatics.as<ITokenBrokerInternalStatics>();
	auto accounts = statics.FindAllAccountsAsync().get();
	wprintf(L"Account count = %i\n", accounts.Size());
	...
	WebAccount accountInfo = accounts.GetAt(0);
	for (int index = 0; index < accounts.Size(); index++) {
		accountInfo = accounts.GetAt(index);
		wprintf(L"accountInfo.Id().c_str() = %s\n", accountInfo.Id().c_str());
		wprintf(L"*accountId = %s\n", *accountId);
		if (wcscmp(accountInfo.Id().c_str(), *accountId) == 0)
			break;
	}
	wprintf(L"ID = %s\n", accountInfo.Id().c_str());
